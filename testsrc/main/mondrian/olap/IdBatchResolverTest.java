...
public class IdBatchResolverTest  extends BatchTestCase {
    ...
    public void testSimpleEnum() {
        assertContains("Resolved map omitted one or more members",
            batchResolve("SELECT "
                + "{[Product].[Food].[Dairy],"
                + "[Product].[Food].[Deli],"
                + "[Product].[Food].[Eggs],"
        ...
        verify(query.getSchemaReader(true), times(2))
    ...
    public void testCalcMemsNotResolved() {
        assertFalse("Resolved map should not contain calc members",
            batchResolve(
    ...
    public void testLevelReferenceHandled() {
        ...
        verify(query.getSchemaReader(true), times(1))
            .lookupMemberChildrenByNames(
                parentMember.capture(),
                childNames.capture(),
                matchType.capture());
        assertEquals("[Time.Weekly].[All Time.Weeklys]",
            parentMember.getAllValues().get(0).getUniqueName());
        assertEquals(
            "[[1997]]",
    ...
    public void testAnalyzerFilterMdx() {
        ...
        verify(query.getSchemaReader(true), times(5))
            .lookupMemberChildrenByNames(
                parentMember.capture(),
                childNames.capture(),
                matchType.capture());

        assertEquals("[Promotions].[All Promotions]",
            parentMember.getAllValues().get(0).getUniqueName());
    ...
    public void testSetWithNullMember() {
        ...
        assertEquals("[Store Size in SQFT].[All Store Size in SQFTs]",
            parentMember.getAllValues().get(0).getUniqueName());
    ...
    public void testMultiHierarchySSAS() {
        ...
        verify(query.getSchemaReader(true), times(2))
            .lookupMemberChildrenByNames(
                ...
                childNames.capture(),
                matchType.capture());
        assertEquals("[Time].[Weekly].[All Weeklys]",
            parentMember.getAllValues().get(0).getUniqueName());
        assertTrue( childNames.getAllValues().get(0).size() == 1);
        assertEquals("1997",
            childNames.getAllValues().get(0).get(0).getName());
    ...
    public void testParentChild() {
        ...
        "WITH\n"
            ...
            + "FROM [HR]\n"
            + "WHERE ([*CJ_SLICER_AXIS])"),
                list( "[Employees].[Sheri Nowmer].[Derrick Whelply]",
        ...
        verify(query.getSchemaReader(true), times(2))
            .lookupMemberChildrenByNames(
                ...
                childNames.capture(),
                matchType.capture());
        assertEquals("[Employees].[Sheri Nowmer]",
            parentMember.getAllValues().get(1).getUniqueName());
    ...
    private String sortedNames(List<Id.NameSegment> items) {
        Collections.sort(items, new Comparator<Id.NameSegment>() {
            @Override
    ...
    {
        return new HashSet(CollectionUtils
            .collect(resolvedIdents.keySet(),
                new Transformer() {
                    @Override
    ...
    public IdBatchResolver makeTestBatchResolver(String mdx) {
        ...
        RolapConnection conn = (RolapConnection) spy(getConnection());
    ...
    private class QueryTestWrapper extends Query {
        ...
        @Override
        public void resolve() {
            // no-op.  Avoids automatic resolution during Query init so
            // we can control it during the test.
        }
