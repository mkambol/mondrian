...
public class SortTest extends FoodMartTestCase {
  ...
  //ORDER(GENERATE(CROSSJOIN([Customers].[Name].MEMBERS,[Product].[Product Family].MEMBERS), {([Customers]
  // .CURRENTMEMBER,[Product].CURRENTMEMBER)}),[Customers].CURRENTMEMBER.ORDERKEY,BASC,ANCESTOR
  //      ([Customers].CURRENTMEMBER,[Customers].[City]).ORDERKEY,BASC,[Measures].[*SORTED_MEASURE],BASC)

  public void __testListTuplesExceedsCellEvalLimit() {
    propSaver.set( MondrianProperties.instance().CellBatchSize, 2 );
    assertAxisReturns(
      "ORDER(GENERATE(CROSSJOIN({[Customers].[USA].[WA].[Port Orchard].[Janell Cordova],[Customers].[USA].[WA].[Puyallup].[Jeremy Styers],[Customers].[USA].[WA].[Spokane].[Glenn Sladnick],[Customers].[USA].[WA].[Spokane].[Bernadette Marschang]},{[Product].[Food]}),\n"
        + "{([Customers].CURRENTMEMBER,[Product].CURRENTMEMBER)}), [Measures].[Store Sales], BASC, [Customers].CURRENTMEMBER.ORDERKEY,BASC)", "" );

  }

  public void __testFoooo() {
    assertQueryReturns( "WITH\n"
      + "SET [*NATIVE_CJ_SET] AS 'CROSSJOIN([*BASE_MEMBERS__Customers_],[*BASE_MEMBERS__Product_])'\n"
