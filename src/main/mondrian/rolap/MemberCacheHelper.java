...
public class MemberCacheHelper implements MemberCache {
        ...
        MemberChildrenConstraint constraint)
    {
        ...
        if (constraint instanceof ChildByNameConstraint) {
            return findNamedChildrenInCache(member,
                ((ChildByNameConstraint) constraint).getChildNames());
        }
    ...
    public synchronized RolapMember removeMember(Object key)
    {
        ...
        mapParentToNamedChildren.getCache().execute(new SmartCache.SmartCacheTask<RolapMember, Collection<RolapMember>>() {
            public void execute(Iterator<Entry<RolapMember, Collection<RolapMember>>> iterator) {
                while (iterator.hasNext()) {
                    Entry<RolapMember, Collection<RolapMember>> entry =
                        iterator.next();
                    RolapMember parentKey = entry.getKey();
                    if (member.equals(parentKey)) {
                        iterator.remove();
                    } else if (parent.equals(parentKey)) {
